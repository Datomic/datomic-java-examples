...
public class Schema {
	...
	public static abstract class Element {
		...
		/**
		 * Return null if this is a comment, otherwise a map that can be
		 * submitted as part of a transaction
		 * 
		 * @return null if this is a comment, otherwise a map that can be
		 *         submitted as part of a transaction
		 */
		...
		public Partition(final String ident) {
			if (ident == null || ident.length() == 0) {
				throw new IllegalArgumentException(
						"Partition ident must be non-empty: " + ident);
			}
			if (!ident.equals(":db.part/db")
					&& !ident.equals("db.part/db")
					&& !ident.equals(":db.part/user")
					&& !ident.equals("db.part/user")
					&& !ident.equals(":db.part/tx")
					&& !ident.equals("db.part/tx")
					&& (ident.startsWith(":db.") || ident.startsWith(":db/")
							|| ident.startsWith("db.") || ident
								.startsWith("db/"))) {
				throw new IllegalArgumentException(
						"Partition ident must not reside in or below the db namespace except for :db.part/{db,user,tx}: "
								+ ident);
			}
			this.ident = ident;
		}
	...
	public static class EnumEntity extends Element {
		...
		public EnumEntity(final String partitionName, final String ident) {
			if (partitionName == null || partitionName.length() == 0
				...
				throw new IllegalArgumentException(
						"partitionName and ident must be non-empty");
			}
			if (!partitionName.equals(":db.part/user")
					&& !partitionName.equals("db.part/user")
					&& (partitionName.startsWith(":db.")
							|| partitionName.startsWith(":db/")
							|| partitionName.startsWith("db.") || partitionName
								.startsWith("db/"))) {
				throw new IllegalArgumentException(
						"Aside from :db.part/user, enum entity partition must not reside in or below the db namespace: "
								+ partitionName);
			}
			if (ident.startsWith(":db.") || ident.startsWith(":db/")
					|| ident.startsWith("db.") || ident.startsWith("db/")) {
				throw new IllegalArgumentException(
						"Enum entity ident must not reside in or below the db namespace: "
								+ ident);
			}
		...
		public String toEdn() {
			final StringBuffer sb = new StringBuffer();
			sb.append("{:db/id #db/id[").append(getPartitionName())
					.append("] :db/ident ").append(getIdent()).append("}\n");
			return sb.toString();
		}
	...
	public static class Attrib extends Element {
		public Attrib(final String ident, final String valueType,
				final String cardinality) {
			if (ident == null || ident.length() == 0 || valueType == null
					|| valueType.length() == 0 || cardinality == null
					|| cardinality.length() == 0) {
				throw new IllegalArgumentException(
						"ident, valueType, and cardinality must all be non-empty: "
								+ ident + ", " + valueType + ", " + cardinality);
			}
			if (ident.startsWith(":db.") || ident.startsWith(":db/")
					|| ident.startsWith("db.") || ident.startsWith("db/")) {
				throw new IllegalArgumentException(
						"ident must not reside in or below the db namespace: "
								+ ident);
			}
			if (!valueType.startsWith(":db.type/")) {
				throw new IllegalArgumentException(
						"valueType must reside in the :db.type namespace: "
								+ valueType);
			}
			if (!cardinality.equals(DB_CARDINALITY_ONE)
					&& !cardinality.equals(DB_CARDINALITY_MANY)) {
				throw new IllegalArgumentException("cardinality must be '"
						+ DB_CARDINALITY_ONE + "' or '" + DB_CARDINALITY_MANY
						+ "': " + cardinality);
			}
			this.ident = ident;
		...
		public Attrib component(final Boolean component) {
			if (component != null && component.booleanValue()
					&& !getValueType().equals(DB_TYPE_REF)) {
				throw new IllegalStateException(
						"Only attributes of valueType=:db.type/ref can be components");
			}
		...
		public Attrib uniqueValue() {
			if ("false".equals(getIndex())) {
				throw new IllegalStateException(
						"Cannot specify :db/unique=:db.unique/value while also explicitly specifying :db/index=false");
			}
			final Attrib retval = new Attrib(this);
		...
		public Attrib uniqueIdentity() {
			if ("false".equals(getIndex())) {
				throw new IllegalStateException(
						"Cannot specify :db/unique=:db.unique/identity while also explicitly specifying :db/index=false");
			}
			final Attrib retval = new Attrib(this);
		...
		public Attrib index(final Boolean index) {
			if (index != null && !index.booleanValue() && getUnique() != null) {
				throw new IllegalStateException(
						"Cannot explicitly specify :db/index=false while also specifying :db/unique="
								+ getUnique());
			}
			final Attrib retval = new Attrib(this);
			retval.index = index;
		...
		public Attrib fulltext(final Boolean fulltext) {
			if (fulltext != null && fulltext.booleanValue()
					&& !getValueType().equals(DB_TYPE_STRING)) {
				throw new IllegalStateException(
						"Cannot specify :db/fulltext=true unless db/valueType=:db.type/string: db/valueType="
								+ getValueType());
			}
			final Attrib retval = new Attrib(this);
	...
	public static EnumEntity enumEntity(final String partitionName,
			final String ident) {
		return new EnumEntity(partitionName, ident);
	}

	public static Object cardinality(Object db, Object attr) {
		return solo(solo(q("[:find ?v " + ":in $ ?attr " + ":where "
				+ "[?attr :db/cardinality ?card] " + "[?card :db/ident ?v]]",
				db, attr)));
	}
	...
	public static boolean hasPartition(final Database db, final String ident) {
		...
		// as per
		// https://groups.google.com/d/msg/datomic/9xxoWH6RxhA/AYVObmrhBp0J
	...
	public static boolean hasSchema(final Database db,
			final String schemaTxAttribIdent, final String schemaName) {
		return hasAttrib(db, schemaTxAttribIdent)
				&& !q(
						...
						db, schemaTxAttribIdent, SCHEMA_TX_ATTRIB_IS_MARKER_TX,
						schemaName).isEmpty();
	}

	@SuppressWarnings("unchecked")
	public static void ensurePartition(final Connection conn,
	...
	public static void ensureSchema(final Connection conn,
			final String schemaName, final List<List<Element>> txData)
	...
	public static void ensureSchema(final Connection conn,
			final String schemaTxAttribIdent, final String schemaName,
			final List<List<Element>> txData) throws InterruptedException,
			ExecutionException {
		if (txData == null || txData.size() == 0
				|| txData.iterator().next().size() == 0) {
			throw new RuntimeException("No transaction data for schema name: "
					+ schemaName);
		}
		...
		Schema.transact(
				conn,
				txData,
				map(":db/id", Peer.tempid(":db.part/tx"), schemaTxAttribIdent,
						schemaName, SCHEMA_TX_ATTRIB_IS_MARKER_TX, false));
		Schema.transact(
				conn,
				list(list()),
				map(":db/id", Peer.tempid(":db.part/tx"), schemaTxAttribIdent,
						schemaName, SCHEMA_TX_ATTRIB_IS_MARKER_TX, true));
	}

	public static void transact(final Connection conn,
			final List<List<Element>> elementTxs,
			final Object... appendsForEachTx) throws InterruptedException,
			ExecutionException {
		for (List<Element> elementTx : elementTxs) {
			final List<Object> tx = new ArrayList<Object>(elementTx.size()
					+ (appendsForEachTx == null ? 0 : appendsForEachTx.length));
			for (Element element : elementTx) {
				final Map<String, Object> txMap = element.toMap();
				if (txMap != null) {
					tx.add(txMap);
				}
			}
	...
	public static String toEdn(final List<List<Element>> elementTxs)
			throws InterruptedException, ExecutionException {
		final StringBuffer sb = new StringBuffer();
		boolean first = true;
		for (List<Element> elementTx : elementTxs) {
			...
			first = false;
			for (Element element : elementTx) {
				final String edn = element.toEdn();
				sb.append(edn);
			}
			sb.append("]");
		}
	...
	private static void ensureSchemaTxAttribs(final Connection conn,
			final String schemaTxAttribIdent) {
		if (!hasAttrib(conn.db(), schemaTxAttribIdent)) {
			try {
				Schema.transact(
						conn,
						list(list(stringAttrib(schemaTxAttribIdent).doc(
								"Name of schema installed by a transaction")
								.index())));
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
		...
		if (!hasAttrib(conn.db(), SCHEMA_TX_ATTRIB_IS_MARKER_TX)) {
			try {
				Schema.transact(
						conn,
						list(list(booleanAttrib(SCHEMA_TX_ATTRIB_IS_MARKER_TX)
	...
	public static void main(String[] args) throws IOException,
		...
		db = conn.db(); // get latest version of the database so we see the
						// change
		System.out.println("Database has a :foo partition now?");
		System.out.println(hasPartition(db, ":foo"));
		System.out
				.println("Ensure that there is a :foo partition -- idempotent");
		ensurePartition(conn, ":foo");
		db = conn.db(); // get latest version of the database so we see the
						// change
		System.out.println("Database has a :foo partition now?");
		...
		elementsTxData.add(stringAttrib(":community/name").fulltext().doc(
				"A community's name"));
		elementsTxData.add(stringAttrib(":community/url").doc(
				"A community's url"));
		elementsTxData.add(refAttrib(":community/neighborhood").doc(
				"A community's neighborhood"));
		elementsTxData.add(manyStringsAttrib(":community/category").fulltext()
				.doc("All community categories"));
		elementsTxData.add(refAttrib(":community/orgtype").doc(
				"A community orgtype enum value"));
		elementsTxData.add(refAttrib(":community/type").doc(
				"A community type enum value"));
		//
		elementsTxData.add(comment(";; community/orgtype enum values"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.orgtype/community"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.orgtype/commercial"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.orgtype/nonprofit"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.orgtype/personal"));
		//
		elementsTxData.add(comment(";; community/type enum values"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/email-list"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/twitter"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/facebook-page"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/blog"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/website"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/wiki"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/myspace"));
		elementsTxData.add(geographyPartition
				.enumEntity(":community.type/ning"));
		//
		elementsTxData.add(comment(";; neighborhood"));
		elementsTxData.add(stringAttrib(":neighborhood/name").uniqueIdentity()
				.doc("A unique neighborhood name (upsertable)"));
		elementsTxData.add(refAttrib(":neighborhood/district").uniqueIdentity()
				.doc("A neighborhood's district"));
		//
		elementsTxData.add(comment(";; district"));
		elementsTxData.add(stringAttrib(":district/name").uniqueIdentity().doc(
				"A unique district name (upsertable)"));
		elementsTxData.add(refAttrib(":district/region").doc(
		...
		System.out
				.println("Here is what the transaction of (a single) partition looks like when you invoke toString():");
		System.out.println(partitionTxData.toString());
		System.out
				.println("Here is what the list of elements in a transaction looks like when you invoke toString():");
		...
		System.out
				.println("Here is what the two transactions look like when you convert them to edn:");
		...
		System.out
				.println(hasAttrib(db, ":community/name") ? "true; cardinality = "
						+ cardinality(db, ":community/name")
						: false);
		System.out.println("Database has a :community/category attribute?");
		System.out
				.println(hasAttrib(db, ":community/category") ? "true; cardinality = "
						+ cardinality(db, ":community/category")
						: false);
		System.out.println("Database has a :bar attribute?");
		System.out.println(hasAttrib(db, ":bar") ? "true; cardinality = "
				+ cardinality(db, ":bar") : false);
		System.out
				.println("Database has a :community.orgtype/community enum entity?");
